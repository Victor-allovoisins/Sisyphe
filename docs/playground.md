...
| 9 | Onglet Réglages, daemon en marche : changer l'intervalle de poll, enregistrer ; changer le label de déclenchement, enregistrer ; relancer le diagnostic, mesurer le disque, vider le cache de build ; saisir un champ invalide (App ID vide), puis vider le budget et enregistrer | L'intervalle s'applique à chaud (aucun bandeau) ; le label déclenche un bandeau « Redémarrage requis » qui reste après un aller-retour d'onglet, avec un bouton Redémarrer (arrêt puis démarrage) ; Diagnostic, Environnement et Dépôts remplis, total disque recalculé après la purge ; l'App ID vide affiche l'erreur sous le champ et n'écrit rien ; budget vidé = « aucune limite », sans jauge sur le tableau de bord |
