...
Run: `npx vitest run test/integration/pipeline.test.ts`
Expected: PASS (17 tests). Si le scénario « annulation » échoue parce que `signal.throwIfAborted()` lève une string et non une Error, c'est attendu : le `catch` ne dépend pas du type de `err`.
...
- Test: `test/integration/daemon.test.ts`, `src/log/logger.test.ts`
...
Expected: PASS (daemon : 9, logger : 5). Le test « watchCancellations » remplace l'agent par un objet ad hoc (`fakeAgent as never`) : le stub est déclaré dans une constante puis affecté, pour que `tsc --noEmit` reste propre.
...
Expected: PASS (retry : 8, client : 18, labels : 2). Si le typecheck bute sur `new App({ ..., Octokit })`, vérifier la signature dans `node_modules/@octokit/app/dist-types/index.d.ts` et ajuster le cast de `getInstallationOctokit` ; ne pas retirer le passage de la classe `Octokit`, c'est ce qui donne `.rest` et `.paginate`.
...
**Files:**
- Create: `src/app.ts`, `src/cli/index.ts`, `src/cli/format.ts`, `src/cli/resolve-job.ts`, `src/daemon/lock.ts`, `src/cli/commands/start.ts`, `src/cli/commands/status.ts`, `src/cli/commands/logs.ts`, `src/cli/commands/report.ts`, `src/cli/commands/cancel.ts`
- Test: `src/cli/format.test.ts`, `src/cli/resolve-job.test.ts`, `src/daemon/lock.test.ts`, `src/cli/commands/logs.test.ts`
...
Expected: PASS (20 tests : format, resolve-job, logs, lock), build sans erreur, l'aide liste `start`, `status`, `logs`, `report`, `cancel`, `doctor`, `setup`.
...
- Test: `src/cli/launchd.test.ts`, `src/cli/checks.test.ts`, `src/cli/commands/setup.test.ts`, `src/cli/commands/doctor.test.ts`
...
Expected: PASS (launchd : 2, checks : 6, setup : 8, doctor : 17), build ok.
...
### Task 26 : backend agent « cli » (Claude Code CLI en mode headless)

#### Pourquoi

L'Agent SDK exige une clé API. Victor n'en a pas : il utilise Claude Code avec l'abonnement Team d'AlloVoisins, et la CLI `claude` (2.1.265, `claude auth status` → loggedIn true, authMethod claude.ai) est installée et connectée sur la machine. On ajoute un second `AgentRunner` qui lance `claude -p` en sous-processus avec les mêmes garde-fous que le runner SDK, sélectionnable par `agentBackend: cli | sdk` dans la config machine. Le runner SDK reste tel quel (utilisé quand une clé API existe).

#### Fichiers

- Create : `src/agent/cli-runner.ts`, `src/agent/cli-runner.test.ts`, `src/agent/path-guard-cli.ts` (script de hook exécuté par la CLI), `test/fakes/fake-claude.sh` (faux binaire `claude` piloté par des variables d'env pour les tests).
- Modify : `src/agent/runner.ts` (remplacer `hooks?: Options['hooks']` par `pathGuard?: { worktreePath: string; protectedPatterns: string[] }`), `src/agent/sdk-runner.ts` (construire les hooks SDK depuis `pathGuard` via `pathGuardHook`), `src/jobs/pipeline.ts` (passer `pathGuard` au lieu de `hooks`), `src/config/machine.ts` (`agentBackend: z.enum(['sdk', 'cli']).default('sdk')`), `src/app.ts` (choisir le runner ; `needsAgent` vérifie la clé API seulement pour `sdk`), `src/cli/commands/doctor.ts` (checks selon backend), `src/cli/commands/setup.ts` (question backend ; clé API demandée seulement pour `sdk` ; plist sans clé pour `cli`), tests associés, `docs/playground.md` §3 et `README.md` (mention du backend).

#### Contrat du runner CLI

`new CliAgentRunner({ claudeBin?: string, hookScript?: string })`. `claudeBin` défaut `'claude'` (résolu sur le PATH de l'env passé). `hookScript` défaut : chemin de `path-guard-cli.js` à côté du module (`new URL('./path-guard-cli.js', import.meta.url)`), donc `dist/agent/path-guard-cli.js` une fois buildé.

`run(o: AgentRunOptions)` :

1. Écrit dans le dossier du transcript (`dirname(o.transcriptPath)`) deux fichiers par appel : `system-append-<n>.md` (contenu de `o.systemPromptAppend`) et `cli-settings-<n>.json`. Le settings JSON contient uniquement, si `o.pathGuard` est défini :
   ```json
   { "hooks": { "PreToolUse": [ { "matcher": "Edit|Write", "hooks": [ { "type": "command", "command": "node <hookScript>", "timeout": 15 } ] } ] } }
   ```
   Le worktree et les motifs protégés sont passés au hook par l'environnement du sous-processus : `SISYPHE_GUARD_WORKTREE` et `SISYPHE_GUARD_PROTECTED` (JSON du tableau).
2. Lance `claude` avec execa, `cwd: o.cwd`, `env: { ...o.env, SISYPHE_GUARD_* }` (ne pas étendre process.env : `o.env` est déjà l'environnement épuré, il contient HOME et PATH, indispensables pour `~/.claude`), `detached: true` (groupe de processus propre, comme `runRepoCommand`), le prompt fourni sur stdin, arguments :
   ```
   -p --output-format stream-json --verbose
   --permission-mode dontAsk --permission-prompts none
   --setting-sources ""            (aucun réglage user/project/local : ni hooks perso, ni CLAUDE.md du repo)
   --settings <cli-settings-n.json>   (seulement si pathGuard)
   --tools <o.allowedTools...>
   --allowedTools <o.allowedTools...>
   --disallowedTools <o.disallowedTools...>   (seulement si non vide)
   --model <o.model>
   --max-turns <o.maxTurns>
   --max-budget-usd <o.maxBudgetUsd>
   --append-system-prompt-file <system-append-n.md>
   --json-schema <JSON.stringify(o.outputSchema)>   (seulement si outputSchema)
   --resume <o.resumeSessionId>                     (seulement si défini)
   ```
   Vérifier avec `claude --help` que `--append-system-prompt-file` existe ; sinon `--append-system-prompt <texte>`.
3. Lit stdout ligne par ligne (JSONL). Chaque ligne parsée est ajoutée telle quelle au transcript (`o.transcriptPath`), comme le runner SDK ; une ligne non JSON est ajoutée sous `{ type: 'sisyphe_raw', data }`. stderr est ajouté sous `{ type: 'stderr', data }`. Mêmes extractions que `SdkAgentRunner.run` : `system/init` → sessionId ; `assistant` (sans `parent_tool_use_id`, dédoublonné par `message.id`) → plancher d'usage ; `result` → capturé. Réutiliser `summarizeResult` de `sdk-runner.ts` en lui passant le `result` parsé (même forme que `SDKResultMessage`) ; exporter au besoin un type structurel minimal.
4. Fin : si `o.signal` est déclenché → tuer le groupe (`process.kill(-pid, 'SIGTERM')`, puis SIGKILL après 5 s) et `aborted: true` ; timeout `o.timeoutMs` → même chose avec `timedOut: true` ; exit non nul sans `result` → `error` avec les 20 dernières lignes de stderr comme message. Ne jamais laisser un `claude` orphelin : `finally` qui tue le groupe si le process vit encore.
5. `sandbox` : non supporté par ce backend ; `app.ts` lève une erreur claire si `machine.sandbox && agentBackend === 'cli'`.

#### Hook `src/agent/path-guard-cli.ts`

Script Node autonome (pas d'import du SDK) : lit tout stdin, `JSON.parse`, prend `tool_input.file_path`, appelle `decidePath(process.env.SISYPHE_GUARD_WORKTREE, filePath, JSON.parse(process.env.SISYPHE_GUARD_PROTECTED ?? '[]'))` (importer `decidePath` depuis `./hooks.js` : attention, `hooks.ts` importe un type du SDK, c'est un `import type`, donc sans effet à l'exécution). Sortie : si refusé, écrit sur stdout `{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"<raison>"}}` et exit 0 ; si autorisé, exit 0 sans sortie. Toute erreur (stdin illisible, env absent) → refus avec raison (ferme par défaut). Test unitaire : lancer le script avec `execa('node', [script])` sur le build ou via `tsx` si disponible ; sinon tester la fonction extraite `guardFromHookInput(json, env)` et garder le script trivial.

#### Faux `claude` pour les tests (`test/fakes/fake-claude.sh`)

Script sh exécutable qui :
- enregistre ses arguments dans `$FAKE_CLAUDE_ARGS_FILE` (un par ligne) et le prompt lu sur stdin dans `$FAKE_CLAUDE_PROMPT_FILE` ;
- émet sur stdout le contenu du fichier `$FAKE_CLAUDE_SCRIPT` (JSONL préparé par le test : init, un assistant, un result) ;
- si `$FAKE_CLAUDE_SLEEP` est défini, dort autant de secondes avant d'émettre le result (pour tester timeout et abort ; lancer `sleep` en enfant pour vérifier que le groupe entier est tué : le test vérifie ensuite que le pid du sleep n'existe plus, écrit dans `$FAKE_CLAUDE_CHILD_PID_FILE`) ;
- code de sortie `$FAKE_CLAUDE_EXIT` (défaut 0).

#### Tests (`src/agent/cli-runner.test.ts`)

1. Arguments : `--output-format stream-json`, `--permission-mode dontAsk`, `--setting-sources` suivi de chaîne vide, `--tools` et `--allowedTools` avec la liste, `--json-schema` avec le schéma sérialisé, `--append-system-prompt-file` pointant sur un fichier dont le contenu est `systemPromptAppend`, `--resume` quand défini et absent sinon, `--settings` présent seulement avec `pathGuard`, prompt reçu sur stdin.
2. Résultat succès : `output` = `structured_output` du result, `sessionId`, `costUsd`, `usage`, `numTurns`, `stopReason 'completed'` ; transcript contient les lignes JSONL.
3. Result `error_max_turns` → `stopReason 'max_turns'` ; exit 1 sans result → `'error'` avec le stderr dans `errorMessage`.
4. Timeout : `FAKE_CLAUDE_SLEEP=5`, `timeoutMs 500` → `'timeout'` en moins de 2 s, plus aucun processus du groupe (le pid enfant a disparu).
5. Abort : `o.signal` déclenché après 200 ms → `'aborted'`, groupe tué.
6. Env : le sous-processus reçoit exactement `o.env` plus `SISYPHE_GUARD_*` (le fake écrit `env` dans `$FAKE_CLAUDE_ENV_FILE`) ; en particulier une variable présente dans `process.env` mais absente de `o.env` n'apparaît pas.
7. Settings JSON : contenu attendu avec le hook `Edit|Write` et la commande `node <hookScript>`.

Adapter `test/helpers/harness.ts` si `pathGuard` remplace `hooks` dans les options (la ScriptedAgentRunner ignore ces champs). Tous les tests existants doivent rester verts (`npm test`), `npx tsc --noEmit` et `npm run build` propres.

#### Câblage

- `machine.ts` : `agentBackend: z.enum(['sdk', 'cli']).default('sdk')`.
- `app.ts` : `agent = machine.agentBackend === 'cli' ? new CliAgentRunner({}) : new SdkAgentRunner({ sandbox: machine.sandbox })` ; la vérification `ANTHROPIC_API_KEY` ne s'applique qu'au backend `sdk` ; erreur si `sandbox` avec `cli`.
- `doctor.ts` : pour `cli`, remplacer les checks clé API par `claude --version` (présent sur le PATH) et `claude auth status` → JSON avec `loggedIn: true` (sinon échec « lancer claude login ») ; garder les checks clé API pour `sdk`.
- `setup.ts` : question « Backend agent (cli = abonnement Claude Code, sdk = clé API) [cli] » ; la clé API n'est demandée et mise dans le plist que pour `sdk`. Le plist pour `cli` n'a pas besoin de la clé mais garde HOME et PATH (dossier du node courant en tête, puis `/opt/homebrew/bin` où vit `claude`).
- `docs/playground.md` §3 et `README.md` : une phrase sur `agentBackend` et la commande `claude auth status`.

#### Validation réelle (une fois, coûte quelques centimes de quota)

Après build : `node dist/cli/index.js doctor` doit afficher ✅ pour les checks CLI. Puis un appel minimal réel pour valider les drapeaux et la forme du result :
```
printf 'Réponds exactement OK' | claude -p --output-format stream-json --verbose --permission-mode dontAsk --permission-prompts none --setting-sources "" --tools "" --max-turns 1 --json-schema '{"type":"object","properties":{"answer":{"type":"string"}},"required":["answer"]}'
```
Vérifier que le dernier message est un `result` avec `structured_output`, `total_cost_usd`, `session_id`, `num_turns`. Coller la forme obtenue (sans contenu sensible) dans le rapport. Si `--setting-sources ""` est refusé, chercher la syntaxe acceptée (`--setting-sources=` ou `--restricted`) et adapter.

#### Commit

`feat(agent): Claude Code CLI backend (headless claude -p) selectable via agentBackend`
