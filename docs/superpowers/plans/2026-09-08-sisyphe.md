...
Expected: PASS (daemon : 8, logger : 4). Le test « watchCancellations » remplace l'agent par un objet ad hoc (`fakeAgent as never`) : le stub est déclaré dans une constante puis affecté, pour que `tsc --noEmit` reste propre.
