...
export async function deliver(i: DeliverInput): Promise<DeliverResult> {
  ...
  const commitSha = await i.git.squashCommit(i.worktreePath, job.branch, job.baseSha, commitMessage(job));
...
export async function runJob(jobId: string, deps: PipelineDeps, signal: AbortSignal): Promise<Job> {
  ...
  try {
    ...
    await deps.git.ensureMirror(job.repo, pushUrl, publicUrl, [defaultBranch]);
    ...
    if (config.baseBranch !== defaultBranch) await deps.git.ensureMirror(job.repo, pushUrl, publicUrl, [config.baseBranch]);
