...
Expected: PASS (retry : 8, client : 16, labels : 2). Si le typecheck bute sur `new App({ ..., Octokit })`, vérifier la signature dans `node_modules/@octokit/app/dist-types/index.d.ts` et ajuster le cast de `getInstallationOctokit` ; ne pas retirer le passage de la classe `Octokit`, c'est ce qui donne `.rest` et `.paginate`.
