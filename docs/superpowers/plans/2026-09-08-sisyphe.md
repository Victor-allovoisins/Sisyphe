...
export function renderPrBody(i: PrBodyInput): string {
  ...
  if (verify.driftedFiles.length) attention.push(`- ℹ️ La vérification (setup/build/test) a modifié des fichiers suivis, non inclus dans ce commit : ${verify.driftedFiles.map((f) => `\`${f}\``).join(', ')}.`);
