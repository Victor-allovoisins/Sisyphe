...
**Files:**
- Create: `src/app.ts`, `src/cli/index.ts`, `src/cli/format.ts`, `src/cli/resolve-job.ts`, `src/cli/commands/start.ts`, `src/cli/commands/status.ts`, `src/cli/commands/logs.ts`, `src/cli/commands/report.ts`, `src/cli/commands/cancel.ts`
- Test: `src/cli/format.test.ts`, `src/cli/resolve-job.test.ts`, `src/cli/commands/logs.test.ts`
...
Expected: PASS (17 tests : format, resolve-job, logs), build sans erreur, l'aide liste `start`, `status`, `logs`, `report`, `cancel`, `doctor`, `setup`.
