...
Principe : un miroir bare par repo (`git clone --mirror`), un worktree par job créé depuis ce miroir. Le token d'accès ne passe que dans l'URL des commandes `fetch` et `push`, jamais dans la config du miroir. Après le clone initial, le fetch ne porte que sur les branches demandées (base du repo) : un refspec miroir `+refs/heads/*:refs/heads/*` échoue avec `refusing to fetch into branch … checked out` dès qu'une branche de job encore extraite dans un worktree (relance après échec, worktree conservé 7 jours) a avancé côté remote. Vérifié expérimentalement.
