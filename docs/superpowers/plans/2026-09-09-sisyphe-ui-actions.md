...
- [ ] **Step 2 : `paused`.** Champ privé + `pause(): DaemonStatus`, `resume(): DaemonStatus`, `status(): DaemonStatus` où `DaemonStatus = { pid, paused, running, queued, startedAt }`. `startNext()` renvoie `null` quand `paused`. `watchCancellations`, `trackPullRequests`, `pollOnce` ne regardent pas `paused`.
...
- [ ] **Step 2 : client** `ControlClient(socketPath)` : `send<T>(cmd, args = {}, source: 'ui' | 'cli'): Promise<CommandResult<T>>`, timeout 5 s ; `ENOENT`/`ECONNREFUSED`/timeout → `DaemonUnreachableError` ; réponse non JSON → `Error`. `isReachable()` = `ping` réussi. `stop` : la réponse arrive avant l'arrêt, ne pas attendre la fermeture du process.
...
- [ ] **Step 4 : `sisyphe cancel`** : si `ControlClient.isReachable()`, envoie `cancel` (`source: 'cli'`) et affiche la réponse ; sinon comportement actuel. Test avec un faux serveur de socket dans un `mkdtemp`.
...
### Task 4 : routes POST, anti-CSRF, démarrage du daemon

**Files:**
- Create: `src/ui/actions.ts` (contrôleur des actions), `src/ui/actions.test.ts`, `src/ui/spawn-daemon.ts`, `src/ui/spawn-daemon.test.ts`
- Modify: `src/ui/server.ts` (+ test), `src/ui/data.ts` (+ test), `src/cli/commands/ui.ts`, `src/cli/index.ts` (`--read-only`)

- [ ] **Step 1 : `UiData.overview()`** : au démarrage de `uiCommand`, lire `PRAGMA user_version` sur la base en lecture seule et refuser de démarrer si elle est inférieure à la version attendue (« base non migrée : lancer `sisyphe start` une fois »), car `openDatabaseReadOnly` ne migre pas et une base v1 ferait échouer `SELECT … FROM actions`. Puis ajoute `daemon.paused: boolean | null`, `control: { reachable: boolean }`, `readOnly: boolean`, `recentActions: ActionRow[]` (20). `UiDataDeps` gagne `actions: ActionStore`, `control: { ping(): Promise<DaemonStatus | null> }` (injectable ; l'implémentation réelle mémorise le résultat 1 s) et `readOnly`. `jobDetail` ajoute `actions: ActionRow[]`.
- [ ] **Step 2 : `spawnDaemon({ paths, client, launchd, spawn, waitMs = 5000 })`** : `readLock` vivant → `{ ok: false, error: 'déjà démarré' }` ; launchd chargé → `launchctl kickstart -k gui/<uid>/<LAUNCHD_LABEL>` ; sinon `spawn(process.execPath, [cliEntry, 'start'], { detached: true, stdio: ['ignore', fd, fd] })` + `unref()` avec `fd` = `logs/daemon-stdout.log` en ajout, `cliEntry` = `new URL('../cli/index.js', import.meta.url)` ; puis boucle `ping` toutes les 250 ms jusqu'à `waitMs` ; échec → `ok: false` avec le tail 20 lignes du log. `spawn`, `launchd` et `client` injectables.
- [ ] **Step 3 : contrôleur** `runAction(name, body, deps)` : valide `name` et le corps avec zod (`cancel`/`retry` : `{ jobId }` ; `enqueue` : `{ repo, issueNumber }` ; autres : `{}`), `start` → `spawnDaemon`, autres → `client.send(name, args, 'ui')`. Traduction : succès → `200 { ok: true, result }` ; `ok: false` → `409` ; `DaemonUnreachableError` → `502` ; zod → `400` ; autre → `500`.
- [ ] **Step 4 : serveur** : accepter `POST` uniquement sur `/api/actions/<name>` ; refus `403` si `readOnly`, si `content-type` ne commence pas par `application/json`, si `x-sisyphe-action !== '1'`, ou si `origin` est présent et n'est pas `http://127.0.0.1:<port>`, `http://localhost:<port>`, `http://[::1]:<port>` ; corps lu avec plafond 16 Ko (413 au-delà). Les autres méthodes non GET restent en 405 avec `Allow: GET, POST` sur cette route.
- [ ] **Step 5 : CLI** : option `--read-only` sur `ui` ; `uiCommand` construit `ActionStore`, `ControlClient(paths.controlSocketPath)` et passe `readOnly`. Message de démarrage : « Sisyphe UI : http://… » (« lecture seule » seulement avec l'option).
- [ ] **Step 6 : tests** : `actions.test.ts` (traduction des codes, validation) ; `server.test.ts` (chaque route POST avec un client factice : 200, 409, 502 ; 403 pour chaque en-tête manquant et pour `Origin` étranger ; 403 en `readOnly` ; 400 corps invalide ; 413 ; overview enrichie) ; `spawn-daemon.test.ts` (verrou vivant → refus ; spawn appelé avec `detached` ; attente bornée avec un `ping` qui ne répond jamais ; succès dès que `ping` répond) ; `data.test.ts` (nouveaux champs, `paused: null` quand injoignable).
- [ ] **Step 7 :** `npm test`, `npx tsc --noEmit`, commit `feat(ui): action routes, csrf guards and daemon spawn`.
...
- Couverture spec : §2.1 (Task 2 + 3), §2.2 (Task 3), §2.3 (Task 4 Step 2), §2.4 (Task 1), §2.5 (Task 4), §2.6 (Task 5), §4 tests répartis par tâche.
