...
- [ ] **Step 2 : bandeau de redémarrage.** Après un enregistrement dont `needsRestart` n'est pas vide, un bandeau nomme les champs concernés. Deux règles :
...
- [ ] **Step 2bis : contrat des réponses.** L'enregistrement renvoie `{ reloaded: true, applied, needsRestart }` quand le daemon a rechargé, `{ reloaded: false, changed }` quand il est arrêté (afficher « prendra effet au démarrage » avec la liste `changed`, et proposer Démarrer, jamais Redémarrer), et `{ reloaded: false, reloadError, changed }` quand le fichier est écrit mais le rechargement refusé (afficher que l'enregistrement a réussi et que le daemon tourne encore sur l'ancienne configuration, avec le message).
- [ ] **Step 3 : blocs.** Diagnostic (bouton Relancer appelant `/api/diagnostics?fresh=1`, une ligne par contrôle avec sa pastille), Espace disque (une ligne par dossier, total, bouton Vider le cache de build avec `confirm()`), Environnement (versions et chemins), Dépôts (accès et validité du `sisyphe.yml`). Chargés au premier affichage de l'onglet, pas au chargement de la page.
