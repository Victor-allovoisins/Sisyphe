...
- [x] **Step 1 : onglet et formulaire.** Quatrième onglet « Réglages ». Champs groupés en Exécution (`pollIntervalSeconds`, `maxConcurrentJobs`, `dailyBudgetUsd` avec un champ vide valant « aucune limite »), Agent (`agentBackend`, `sandbox`), GitHub (`appId`, `installationId`, `privateKeyPath`, `triggerLabel`, `repos` en liste avec ajout et retrait). `dataDir` affiché désactivé avec la mention « se change par sisyphe setup ». Enregistrer inactif tant que rien n'a changé ; erreurs de validation affichées sous le champ concerné à partir de `issues`.
- [x] **Step 2 : bandeau de redémarrage.** Après un enregistrement dont `needsRestart` n'est pas vide, un bandeau nomme les champs concernés. Deux règles :
  - **Redémarrer n'est proposé que si le daemon répond** (`control.reachable`). Il enchaîne `stop` puis `start`, et lancer `stop` sur un daemon arrêté échouerait. Daemon arrêté, le bandeau dit simplement que les modifications prendront effet au prochain démarrage.
  - **Le rappel est persistant.** Il ne vient pas de la seule réponse de l'enregistrement, qui disparaît au rafraîchissement suivant, mais de `daemon.pendingRestart` exposé par le statut du daemon : le rappel reste affiché tant que le daemon tourne sur une configuration dépassée, y compris après avoir quitté puis rouvert l'onglet, et deux enregistrements rapprochés ne produisent pas un second bandeau vide.
- [x] **Step 2bis : contrat des réponses.** L'enregistrement renvoie `{ reloaded: true, applied, needsRestart }` quand le daemon a rechargé, `{ reloaded: false, changed }` quand il est arrêté (afficher « prendra effet au démarrage » avec la liste `changed`, et proposer Démarrer, jamais Redémarrer), et `{ reloaded: false, reloadError, changed }` quand le fichier est écrit mais le rechargement refusé (afficher que l'enregistrement a réussi et que le daemon tourne encore sur l'ancienne configuration, avec le message).
- [x] **Step 3 : blocs.** Diagnostic (bouton Relancer appelant `/api/diagnostics?fresh=1`, une ligne par contrôle avec sa pastille), Espace disque (une ligne par dossier, total, bouton Vider le cache de build avec `confirm()`), Environnement (versions et chemins), Dépôts (accès et validité du `sisyphe.yml`). Chargés au premier affichage de l'onglet, pas au chargement de la page.
- [x] **Step 4 : lecture seule.** L'onglet reste consultable, tous les champs et boutons d'action désactivés ou masqués, comme le bandeau système.
- [x] **Step 5 : tests page.** Présence de l'onglet, des champs, du bandeau, des quatre blocs ; désactivation complète en lecture seule ; toujours zéro `innerHTML` ; le corps du script se compile avec `new Function`.
...
- [x] **Step 7 :** `npm test`, commit `feat(ui): settings tab`.
