...
- Installe ce qui manque, dans l'ordre : `git` ; Node ≥ 24 (macOS : `brew install node` ; Ubuntu : dépôt NodeSource `setup_24.x` puis `apt-get install -y nodejs`, seules commandes exécutées via `sudo`) ; `gitleaks` (macOS : `brew install gitleaks` ; Ubuntu : archive de la dernière release GitHub pour l'architecture, somme vérifiée avec le `checksums.txt` de la release, binaire posé dans `~/.local/bin`, ajouté au `PATH` du profil shell si absent) ; la CLI Claude Code (`npm install -g @anthropic-ai/claude-code`) si `claude` est absent. Homebrew absent sur macOS → message avec la commande officielle et arrêt.
- Compile et lie : `npm ci && npm run build && npm link` dans le répertoire du script (le service pointera sur ce clone via le lien global `sisyphe`).
...
- Relance = mise à jour : `git pull --ff-only` si l'arbre est propre (`--no-pull` pour sauter), puis rebuild ; `sisyphe setup` n'est pas relancé si la config existe (`sisyphe setup --reinstall-service` réécrit l'unité si elle a changé).
