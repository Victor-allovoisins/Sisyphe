...
- Installe ce qui manque, dans l'ordre : `git` ; Node ≥ 24 (macOS : `brew install node` ; Ubuntu : script NodeSource `setup_24.x` téléchargé dans un fichier temporaire puis exécuté (`curl -o` puis `sudo -E bash <fichier>`, jamais un tube vers `sudo` : un téléchargement tronqué ne doit pas s'exécuter à moitié en root) puis `apt-get install -y nodejs` ; avec `apt-get install -y git`, ce sont les seules commandes exécutées via `sudo`) ; `gitleaks` (macOS : `brew install gitleaks` ; Ubuntu : archive de la dernière release GitHub pour l'architecture, somme vérifiée avec le `checksums.txt` de la release, binaire posé dans `~/.local/bin`, ajouté au `PATH` du profil shell si absent) ; la CLI Claude Code (`npm install -g @anthropic-ai/claude-code`) si `claude` est absent. Homebrew absent sur macOS → message avec la commande officielle et arrêt.
...
- `sudo` uniquement pour `apt-get` (Node et git) sur Ubuntu, jamais pour `npm` ; tout le reste s'installe dans le compte utilisateur. Le binaire gitleaks est vérifié par la somme publiée avec la release.
