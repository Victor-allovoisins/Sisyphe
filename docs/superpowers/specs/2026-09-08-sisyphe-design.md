...
- **Auth Claude : deux backends** sélectionnés par `agentBackend` dans la config machine. `sdk` : Claude Agent SDK avec une clé API (`ANTHROPIC_API_KEY`), coût réel par issue, sandbox possible. `cli` : la CLI Claude Code installée sur la machine, lancée en mode headless (`claude -p`) avec l'authentification de son utilisateur (abonnement Claude Code), mêmes garde-fous (liste blanche d'outils, hook de garde des chemins, aucun réglage user/project/local, budget, tours, timeout, transcript JSONL, sortie structurée) ; le coût affiché est l'estimation renvoyée par la CLI ; pas de sandbox. L'abonnement Claude Code du développeur n'est jamais utilisé via le SDK ; la clé API n'est pas transmise au processus `claude` en backend `cli`.
...
Pour chaque candidate, `canTrigger` vérifie que le dernier événement `labeled` du label trigger a été posé par un utilisateur ayant la permission write, maintain ou admin sur le repo. Si aucun événement n'existe alors que le label est présent (label posé à la création de l'issue), c'est l'auteur de l'issue qui est vérifié. Un label posé par un bot est refusé ; une réponse 4xx lors de la vérification vaut refus, tandis qu'une erreur réseau ou 5xx laisse l'issue de côté pour le cycle suivant (pas de retrait de label, pas de commentaire). Sinon : commentaire « label ignoré, permission insuffisante », retrait du label, pas de job.
...
3. Git : miroir nu `~/.sisyphe/mirrors/<owner>__<repo>.git` créé avec `git init --bare` la première fois, puis fetch ciblé des branches de base dans l'espace `refs/sisyphe/base/<branche>` (jamais extraites, donc toujours rafraîchissables et hors de portée de l'agent) ; le token n'apparaît que sur la ligne de commande du fetch/push, jamais dans la config ni dans `FETCH_HEAD`. Worktree `~/.sisyphe/work/<owner>__<repo>/issue-<n>` sur une branche neuve `<branchPrefix>issue-<n>-<slug>` depuis `refs/sisyphe/base/<baseBranch>`. Le slug est le titre en kebab-case tronqué à 40 caractères. Le SHA de la base au moment de la création est enregistré comme `base_sha` du job : c'est la référence du squash final, même si la base avance pendant le job. Si la branche existe déjà sur le remote (job précédent échoué), elle est écrasée au push : chaque job repart de la base.
...
2. Scan de secrets (gitleaks) sur les lignes ajoutées du patch, avant toute commande : bloquant, job `failed` immédiat, aucun push, commentaire sur l'issue avec les fichiers et règles concernés sans le contenu. L'arbre vérifié (`write-tree`) est capturé à ce moment : c'est exactement lui qui sera livré, même si le build modifie ensuite des fichiers (lockfiles).
3. `commands.setup` rejoué (le projet a pu changer, par exemple un fichier ajouté sous xcodegen), puis `commands.build`, `commands.test`, `commands.lint`, chacune si définie, avec timeout global `timeouts.verifyMinutes` (défaut 30). Sorties capturées dans le dossier du job.
4. Échec et `attempt < limits.maxAttempts` (défaut 3) : retour en `implementing` avec `resumeSessionId` et un prompt « la vérification a échoué, voici les 200 dernières lignes, corrige ». Échec à la dernière tentative : flag `verificationFailed`, passage en `delivering` pour une PR draft d'inspection, issue finale `failed` (voir 4.7).
5. Flags calculés sur le diff : `protectedPathsTouched` (globs de base non désactivables plus `protectedPaths`), `largeDiff` (plus de `limits.maxDiffLines` lignes modifiées, défaut 800), `driftedFiles` (fichiers suivis modifiés par le build après la capture de l'arbre, informatif).

### 4.7 Livraison

1. Squash en un commit unique construit avec `git commit-tree` sur l'arbre capturé à la vérification, parent `base_sha` (HEAD du worktree épinglé pour que l'agent ne puisse pas déplacer la base). Message : `<change_type>(#<n>): <titre de l'issue>`, corps `Closes #<n>` et `Co-Authored-By: Sisyphe <sisyphe[bot]@users.noreply.github.com>`.
...
Fichiers par job dans `~/.sisyphe/jobs/<id>/` : `transcript-<phase>-<attempt>.jsonl` (tous les messages SDK), `setup.log`, `build.log`, `test.log`, `lint.log`, `diff.patch`, `gitleaks.json` le cas échéant.
...
- `sisyphe setup` : interactif. Valide chaque réponse avant d'écrire quoi que ce soit (repos au format owner/repo, identifiants entiers, clé privée lisible, clé API non vide, jamais affichée), fusionne avec un `config.yml` existant (les réglages non demandés sont conservés), écrit `config.yml` en 0600, crée les dossiers de données et la base, exécute les vérifications de `doctor` et, seulement si elles passent, **installe le service sans le démarrer** via `src/service/` (plist launchd, unité systemd, ou message d'aide sur une plateforme sans service géré) ; `--reinstall-service` réécrit l'unité sans reposer les questions. Refuse de s'installer si le CLI n'est pas exécuté depuis le build (`dist/cli/index.js`). Détail du service, du fichier `enabled` et des permissions : `2026-09-10-sisyphe-install-service-design.md` §3 et §4, qui remplace la description launchd de la v1. Compromis assumé pour le POC, avec le backend `sdk` uniquement : la clé API est recopiée dans le plist ou l'unité (0600) et visible par les processus du même utilisateur ; le backend `cli`, désormais le défaut, n'écrit aucun secret.
- `sisyphe doctor` : vérifie node ≥ 24, git, gitleaks, la clé API et sa validité pour le backend `sdk` ou `claude auth status` pour le backend `cli`, la config machine, l'accès de la GitHub App à chaque repo, la présence et la validité de `sisyphe.yml` sur la branche par défaut de chaque repo, la présence sur le PATH du premier mot de chaque commande déclarée, la longueur du chemin de la socket de contrôle, et l'état du service ; il **échoue** sur un agent launchd d'une version antérieure (`RunAtLoad true` ou absence de `PathState`), qui ferait mentir l'état « au boot » et empêcherait un arrêt de tenir. En avertissement seulement : `caffeinate` absent (macOS), moins de 10 Go libres, service non installé, `linger` désactivé (Linux). Chaque échec de prérequis cite la commande d'installation de la plateforme. Sortie : une ligne par vérification (✅ ⚠️ ❌), code de retour non nul si une vérification bloquante échoue.
...
- **End-to-end manuel** : un repo réel à build rapide (`Ashentale/Ashentale`, iOS avec xcodegen, `sisyphe.yml` sur `main`), la vraie GitHub App et le backend agent réel (`cli` avec la CLI connectée, ou `sdk` avec une clé API). Fumée obligatoire avant de brancher ILokYou-iOS.
