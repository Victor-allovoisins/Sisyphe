...
- `codex` : la CLI OpenAI Codex (`codex exec`), pour l'abonnement ChatGPT. Authentification : la session de la CLI (`codex login status`, « Logged in using ChatGPT ») ; `OPENAI_API_KEY` est retiré de l'environnement du run pour éviter la bascule silencieuse sur l'API facturée. La CLI doit être installée et joignable sur le `PATH` du service.
