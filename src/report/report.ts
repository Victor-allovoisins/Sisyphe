...
export function parseSince(text: string, now: Date = new Date()): string {
  ...
  const invalid = () => new Error(`Durée invalide : ${text} (attendu par exemple 12h, 30d, 2w)`);
  if (!m) throw invalid();
  const n = Number(m[1]);
...
export function buildReport(jobs: Job[], sinceIso: string): ReportStats {
  ...
  const totalCostUsd = jobs.reduce((s, j) => s + j.costUsd, 0);
  return {
    since: sinceIso,
    total: jobs.length,
    byState,
    ...
    costPerMergedPrUsd: merged.length ? totalCostUsd / merged.length : null,
    recentFailures: jobs
...
export function renderReportMarkdown(s: ReportStats): string {
  ...
  const header = `# Sisyphe — rapport depuis ${s.since.slice(0, 16).replace('T', ' ')}`;
  if (s.total === 0) return [header, '', 'Aucun job sur la période.'].join('\n');
  const costPerMergedPr = s.costPerMergedPrUsd === null ? 'n/a' : `$${s.costPerMergedPrUsd.toFixed(2)}`;
  const lines = [
    header,
    '',
    `- Jobs : ${s.total}`,
    `- Terminés (hors annulés) : ${s.finished}`,
    `- PR ouvertes : ${s.withPr} (${pct(s.prOpenedRate)} des jobs terminés, dont ${s.byState.blocked} bloqués au triage)`,
    `- PR mergées : ${s.merged} (${pct(s.prMergedRate)} des PR ouvertes)`,
    `- Coût par PR mergée : ${costPerMergedPr}`,
  ...
  if (s.recentFailures.length) {
    lines.push(
      ...
      '## Derniers échecs',
      '',
      ...s.recentFailures.map((f) => `- ${f.repo}#${f.issueNumber} (${f.id.slice(0, 8)}) : ${f.error ? oneLine(f.error) : 'sans détail'}`),
    );
  }
  return lines.join('\n');
}
