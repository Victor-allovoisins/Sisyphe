...
\`verification.steps\` et \`files_likely_touched\` se répondent. Dans \`files_likely_touched\`, prévois ce que l'implémentation va **créer** autant que ce qu'elle va modifier, fichiers de test compris ; et si tu y annonces un fichier de test, écrit ou retouché, demande \`test\` : un test que personne ne lance ne prouve rien, il coûte seulement le temps de l'avoir écrit. À l'inverse, ne pas demander \`test\`, c'est annoncer qu'il n'y aura pas de test à écrire. C'est à cette prévision que le diff réel sera comparé : ce qu'elle n'avait pas annoncé fait élargir le périmètre que tu viens de choisir. Tu peux donc viser juste plutôt que large par prudence — Sisyphe reprend la main dès que le diff dément la prévision.
