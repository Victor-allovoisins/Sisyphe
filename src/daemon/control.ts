...
async function execute(req: ControlRequest, opts: ControlServerOptions, log: Logger): Promise<CommandResult<unknown>> {
  ...
  try {
    switch (req.cmd) {
      ...
      case 'reload':
        // Journalisée par le daemon lui-même, comme les autres commandes qui passent par sa porte.
        return await daemon.reload(req.source);
      case 'purge':
        // Idem : refus pendant un job, suppression et journal, tout se passe derrière la porte du daemon.
        return await daemon.purgeBuildCache(req.source);
      case 'cancel':
