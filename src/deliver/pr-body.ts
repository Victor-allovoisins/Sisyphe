...
const STEP_LABEL: Record<VerifyStep['status'], (s: VerifyStep) => string> = {
  ...
  skipped: () => '⏭️ non exécutée (étape précédente en échec ou budget épuisé)',
};
...
export function sanitizeReport(r: ImplementationReport): ImplementationReport {
  const s = sanitizeModelText;
  return {
    ...r,
    summary: s(r.summary),
    changes: r.changes.map((c) => ({ file: s(c.file), what: s(c.what) })),
...
export function renderPrBody(i: PrBodyInput): string {
  ...
  const attention = bullets(report.risks);
  ...
  if (attention.length) lines.push("## Points d'attention", '', ...attention, '');
