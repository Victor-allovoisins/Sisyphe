import { access, chmod, constants, copyFile, rename, rm } from 'node:fs/promises';
...
export async function validateMachineConfigInput(raw: unknown, current: MachineConfig): Promise<ValidateMachineConfigResult> {
  ...
  // Seul le chemin circule : le contenu de la clé n'est ni lu, ni affiché, ni transmis.
  try {
    await access(expandHome(config.github.privateKeyPath), constants.R_OK);
  } catch {
...
/**
 * Écrit la configuration machine sans jamais laisser le fichier à moitié remplacé : copie de la version
 * précédente en `<path>.bak`, écriture dans un temporaire du même dossier — donc du même système de
 * fichiers, seule condition pour que `rename` soit atomique — puis `rename`. Le tout en 0600.
 *
...
export async function writeMachineConfig(path: string, config: MachineConfig): Promise<void> {
  ...
  try {
    ...
    // Première écriture : il n'y a pas encore de version à sauvegarder.
    if ((err as NodeJS.ErrnoException).code !== 'ENOENT') throw err;
    backedUp = false;
  }
  // `copyFile` reprend le mode de la source, qui peut avoir été relâché à la main : on le referme.
  if (backedUp) await chmod(backup, 0o600);
